...
window.LEARN_TRANSLATIONS = {
  en: [
    ...
    },
    {
      ...
      public: "On your laptop, one Q-learning step costs about **10⁻¹¹ CHF** (one hundredth of a billionth). A full default Mission III run costs a fraction of a centime — basically free. But the total cost depends on **how many steps** the algorithm needs, and that number explodes with the size of the state space. Modern methods bypass tabular enumeration by *generalising* with neural networks — a function maps states to values, instead of one entry per state. DQN (Mnih et al. 2015) does this for Q-learning on Atari. AlphaZero (Silver et al. 2018) goes further on chess (~10⁴⁴ legal positions) by combining a network (policy + value) with tree search — not Q-learning at all, but a different algorithmic family.",
  ...
  it: [
    ...
    },
    {
      ...
      public: "Sul tuo laptop, un passo di Q-learning costa circa **10⁻¹¹ CHF** (un centesimo di miliardesimo). Una sessione completa di Missione III costa una frazione di centesimo. Ma il costo totale dipende da **quanti passi** l'algoritmo richiede, e quel numero esplode con la dimensione dello spazio degli stati. I metodi moderni evitano l'enumerazione tabulare *generalizzando* tramite reti neurali — una funzione mappa stati a valori, invece di una voce per stato. DQN (Mnih et al. 2015) fa questo per il Q-learning su Atari. AlphaZero (Silver et al. 2018) va oltre sugli scacchi (~10⁴⁴ posizioni legali) combinando una rete (politica + valore) con la ricerca ad albero — non è Q-learning, ma una famiglia diversa di algoritmi.",
