...
window.LEARN_TRANSLATIONS = {
  en: [
    ...
    },
    {
      ...
      math: "Marginal cost per Q-update ≈ 10⁻¹¹ CHF (dominated by amortised hardware, not electricity). Default Mission III: ~10⁶ steps → ~10⁻⁵ CHF. For chess (~10⁴⁴ legal positions), even at 10⁻¹¹ CHF/step a tabular Q is unreachable: it would require orders of magnitude more steps than any machine could perform. **AlphaZero's solution (Silver et al. 2018):** approximate Q*(s,a) by a neural network f_θ(s,a). Benefits: (1) memory is constant in θ, independent of |S|; (2) the function generalises — a state close to a visited one inherits a reasonable value. AlphaZero's training cost was not disclosed by DeepMind; external estimates put it in the millions to tens of millions of USD in TPU compute.",
    ...
    },
    {
      ...
      math: "Total step budget B = E × H. The two extremes:\n• High E, low H (many short episodes) — explores the vicinity of start states; low variance estimates.\n• Low E, high H (few long episodes) — explores deep into the state graph; richer reward propagation per episode.\n\n**Threshold**: H ≥ G (God's number) is mandatory. If H < G the worst-case start state is unreachable in one episode → no reward signal → no learning, regardless of how big B is. M1: G=6, H=30 (5×). M2: G=21, H=60 (~3×). M3: G=31, H=120 (~4×). The c-factor is generous to accommodate ε-greedy meandering.",
  ...
  de: [
    ...
    },
    {
      ...
      math: "Gesamt-Schrittbudget B = E × H. Die zwei Extreme:\n• Viele E, kleines H (viele kurze Episoden) — erkundet die Umgebung der Startzustände; niedrige Varianz.\n• Wenige E, großes H (wenige lange Episoden) — dringt tief in den Zustandsgraphen ein; reichere Belohnungs-Propagation pro Episode.\n\n**Schwelle**: H ≥ G (Gottes Zahl) ist zwingend. Bei H < G ist der schlimmste Startzustand in einer Episode unerreichbar → kein Belohnungssignal → kein Lernen, egal wie groß B ist. M1: G=6, H=30 (5×). M2: G=21, H=60 (~3×). M3: G=31, H=120 (~4×). Der c-Faktor ist großzügig, um ε-greedy-Wanderungen zu erlauben.",
